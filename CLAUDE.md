...
│   ├── lib/
│   │   ├── normalize.ts # URL normalization/validation
│   │   ├── fetchPage.ts  # Capped, streamed page fetch + signal extraction
│   │   └── detect.ts      # Fingerprint-matching engine over the bundled dataset
│   └── fingerprints/
│       ├── categories.json # webappanalyzer category names
│       └── technologies.json # webappanalyzer fingerprint dataset (~7.5k techs)
├── tests/
│   └── *.test.ts      # Unit tests
...
npm run build                # Compile TypeScript to JavaScript + copy fingerprints to dist/
npm start                      # Run compiled server
npm run update-fingerprints      # Refresh src/fingerprints/*.json from upstream
```
...
- `configSchema.source`: Where to extract config schema from — `code` means MCPize
  reads the exported `configSchema` Zod object in `src/index.ts`. This server's only
  configured field, `userAgent`, is optional and maps to the `USER_AGENT` environment
  variable at runtime.

## Environment Variables

- `PORT`: Server port (default: 8080, set by MCPize)
- `USER_AGENT`: Optional. Overrides the default User-Agent sent when fetching pages.
  Unset by default.
